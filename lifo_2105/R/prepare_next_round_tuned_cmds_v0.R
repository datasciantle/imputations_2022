...
base_s3_path <- 's3://datasci-scantle/unified_ncs/experiments/2105_tuner'
...
s3_previous_imputation <- 's3://datasci-scantle/unified_ncs/experiments/2105_tuner/iteration_2/inputs/imputed_results_from_previous_round_csv'
...
cmd_base <- 'sudo docker run --net=host --rm -e SEED_PATH=s3://datasci-scantle/seed/docker/ -v /tmp:/tmp 126345656468.dkr.ecr.us-east-1.amazonaws.com/quantumplethodon:rkumar-tf-001 python3 /tmp/docker/lgbm_dmf_imputer_focal_loss_nextIteration_tuned_v0.py --base_s3_path this_base_s3_path --model_version this_model_version --s3_attribute_matrix this_s3_attribute_matrix --s3_distributions this_s3_distributions --s3_previous_imputation this_s3_previous_imputation --s3_valid_respondents this_s3_valid_respondents --model this_model --iteration this_iteration --hyperopt_max_evals 20'
...
completed <- system(paste0('aws s3 ls ' , base_s3_path, '/iteration_',iteration,'/completeness/'),intern=TRUE)
completed <- as.vector(sapply(completed, function(x) tail(strsplit(x,' ')[[1]],1)))
