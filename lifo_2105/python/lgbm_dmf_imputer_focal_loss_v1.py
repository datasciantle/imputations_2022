...
if multi:
    latent_distribution_df.loc[((labeled_indices) & (latent_distribution_df['label']== -1.)),'label'] = 0.0
...
these_results.to_csv(local_set_predictions_file, columns=['uid','set_uid','is_labeled','label','max_prediction','selected_value'], index=False)
these_results.to_csv(local_set_probabilities_file, columns=['uid','is_labeled','label'] + [str(x) for x in np.arange(num_class)], index=False)
