...
parser.add_argument('--hyperopt_max_evals', default=2, type=int,
...
taxonomy = labels[['attribute','sakey','svkey','multi','label']].drop_duplicates()
taxonomy.label = taxonomy.label.astype(int)
...
labeled_weights = valid_respondents.merge(labels.loc[(labels.label.notnull())][['uid','label']], left_on=['uid'],right_on=['uid'])
taxonomy = taxonomy.merge(distributions_df, left_on=['sakey','svkey','multi'], right_on=['sakey','svkey','multi'], how='left')
if multi:
    taxonomy = taxonomy.loc[taxonomy.label==1.]
...
raw_feature_columns = [x for x in previous_imputation if not x in ['uid','set_uid','wavekey','label']]
...
num_class = len(np.unique(labeled_weights.label.values))
boolean_options = [True,False]

# Bootstrap distribution CIs
simulated_distributions =[]
for sim in np.arange(1000):
    data = labeled_weights[['wavekey','label','weight']]
...
def score_impute_focal_loss(impute_params, X_train, y_train, X_test, y_test, ignore_fail=False):
    ...
    train_data = lgb.Dataset(data=X_train[feature_columns], label=y_train, feature_name=feature_columns, categorical_feature=feature_columns, free_raw_data=True, params={'verbose':-1}, init_score=np.full_like(y_train, fl.init_score(y_train), dtype=float))
    test_data = lgb.Dataset(data=X_test[feature_columns], label=y_test, feature_name=feature_columns, categorical_feature=feature_columns, free_raw_data=True, params={'verbose':-1}, init_score=np.full_like(y_test, fl.init_score(y_train), dtype=float), reference=train_data)
...
if multi:
    ...
    X_train = previous_imputation.loc[train_set_indices]
    X_test = previous_imputation.loc[test_set_indices]
    ...
    try:
        trials = Trials()
        fmin_this_treatment = partial(score_impute_focal_loss, X_train=X_train, y_train=y_train_binary, X_test=X_test, y_test=y_test_binary, ignore_fail=False)
        ...
        trials = Trials()
        fmin_this_treatment = partial(score_impute_focal_loss, X_train=X_train, y_train=y_train_binary, X_test=X_test, y_test=y_test_binary, ignore_fail=True)
        best = fmin(fmin_this_treatment,  space=space_lgb, algo=tpe.suggest, max_evals=args.hyperopt_max_evals , trials=trials, show_progressbar=True)
...
if  multi:
    ...
    these_results = previous_imputation[['uid','wavekey','set_uid','label']]
...
if not multi:
    ...
    predictions = pd.DataFrame(predictions_dict)
    these_results = previous_imputation[['uid','set_uid','label','wavekey']]
    these_results['attribute_model'] = model
    these_results = pd.concat([these_results, predictions], axis=1)
...
these_results = these_results.merge(valid_respondents[['uid','weight']], left_on=['uid'], right_on=['uid'])
for wave in waves:
...
wave_distribution_df['value'] = wave_distribution_df.index
wave_distribution_df = taxonomy.merge(wave_distribution_df, left_on=['label'], right_on=['value'])

labeled_data = previous_imputation.loc[labeled_indices][raw_feature_columns]
initial_feature_selection = feature_selection.chi2(previous_imputation.loc[labeled_indices][raw_feature_columns], previous_imputation.loc[labeled_indices,'label'])
post_feature_selection = feature_selection.chi2(previous_imputation[raw_feature_columns], these_results['selected_value'])
ifs_df = pd.concat([pd.DataFrame(raw_feature_columns, columns=['attribute']),pd.DataFrame(initial_feature_selection[0], columns=['chi2']), pd.DataFrame(initial_feature_selection[1], columns=['p_value'])],axis=1)
ifs_df['attribute_model'] = args.model
pfs_df = pd.concat([pd.DataFrame(raw_feature_columns, columns=['attribute']),pd.DataFrame(post_feature_selection[0], columns=['chi2']), pd.DataFrame(post_feature_selection[1], columns=['p_value'])],axis=1)
