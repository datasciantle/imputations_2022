...
val randomAudiencesExist = FileSystem.get(new java.net.URI(params.outDir + "summaries/multivariate_distributions/randomly_chosen_audiences"), sc.hadoopConfiguration).
  exists(new Path(params.outDir + "summaries/multivariate_distributions/randomly_chosen_audiences"))

val randomly_chosen_audiences: org.apache.spark.sql.DataFrame = if(!randomAudiencesExist){
  ...
  spark.read.load(params.outDir + "summaries/multivariate_distributions/randomly_chosen_audiences")
  } else {
    spark.read.load(params.outDir + "summaries/multivariate_distributions/randomly_chosen_audiences")
  }
...
val fsvksDFExists = FileSystem.get(new java.net.URI(params.outDir + "summaries/multivariate_distributions/randomly_chosen_svkey_avkey"), sc.hadoopConfiguration).
  exists(new Path(params.outDir + "summaries/multivariate_distributions/randomly_chosen_svkey_avkey"))

val fsvksDF: org.apache.spark.sql.DataFrame = if(!fsvksDFExists){
  ...
  val fsvksDF_1 = spark.read.jdbc(jdbcUrl, getAttributeValueKeysFromSurveyValueKeys.replace("SVKEYS", focal_svkeys.mkString(",")), connectionProperties)
  fsvksDF_1.write.mode("overwrite").save(params.outDir + "summaries/multivariate_distributions/randomly_chosen_svkey_avkey")
  spark.read.load(params.outDir + "summaries/multivariate_distributions/randomly_chosen_svkey_avkey")
  } else {
    spark.read.load(params.outDir + "summaries/multivariate_distributions/randomly_chosen_svkey_avkey")
  }
...
val random_audiences_donzo = random_audiences.par.map(avks => {
  ...
  val platformResultExists = FileSystem.get(new java.net.URI(params.outDir + "summaries/multivariate_distributions/randomly_chosen_platform_results/" + aud), sc.hadoopConfiguration).
  exists(new Path(params.outDir + "summaries/multivariate_distributions/randomly_chosen_platform_results/" + aud))
  if(!platformResultExists){
    println("need to run")
